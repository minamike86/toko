...
## Unit & Measurement (Step 6.5)

Setiap ProductVariant wajib memiliki canonical unit.

Canonical unit adalah satuan internal resmi yang digunakan oleh seluruh sistem
untuk representasi quantity.

Aturan:

- Setiap ProductVariant harus memiliki tepat satu canonical unit
- Canonical unit tidak boleh ambigu
- Canonical unit tidak boleh berubah jika sudah memiliki histori

Sistem juga mendukung input unit tambahan yang dapat dikonversi ke canonical.

Conversion rule:

- Bersifat one-way: input → canonical
- Harus eksplisit
- Tidak boleh implicit atau fallback
- Tidak boleh chaining antar unit

Catalog adalah satu-satunya domain yang berwenang
mendefinisikan unit dan conversion rule.

Pada tahap ini, canonical unit dimiliki oleh ProductVariant,
bukan Product.

Product hanya bersifat grouping konseptual.

Semua operasi yang menyangkut quantity dan unit
menggunakan ProductVariant sebagai identity utama.

Jika conversion rule tidak ditemukan:

→ sistem wajib melempar domain/application error
→ mutation inventory tidak boleh dipanggil
→ tidak boleh ada fallback unit

Canonical unit tidak boleh diubah jika:

- ProductVariant sudah memiliki StockMovement, atau
- sudah pernah digunakan dalam transaksi Procurement

Perubahan canonical unit setelah kondisi tersebut
harus dianggap sebagai breaking change
dan hanya boleh dilakukan melalui mekanisme migrasi resmi.
